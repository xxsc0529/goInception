...
func (s *testParserSuite) TestDDL(c *C) {
	...
	table := []testCase{
		...
		// OceanBase DYNAMIC_PARTITION_POLICY (official syntax)
		{"CREATE TABLE test_tbl1 (col1 INT, col2 TIMESTAMP) DYNAMIC_PARTITION_POLICY(ENABLE = true, TIME_UNIT = 'hour', PRECREATE_TIME = '3 hour', EXPIRE_TIME = '1 day', TIME_ZONE = '+8:00', BIGINT_PRECISION = 'none') PARTITION BY RANGE (col2) (PARTITION P0 VALUES LESS THAN (TIMESTAMP '2024-11-11 13:30:00'))", true, "CREATE TABLE `test_tbl1` (`col1` INT,`col2` TIMESTAMP) DYNAMIC_PARTITION_POLICY (ENABLE = true, TIME_UNIT = 'hour', PRECREATE_TIME = '3 hour', EXPIRE_TIME = '1 day', TIME_ZONE = '+8:00', BIGINT_PRECISION = 'none') PARTITION BY RANGE (`col2`) (PARTITION `P0` VALUES LESS THAN (TIMESTAMP '2024-11-11 13:30:00'))"},
		{"CREATE TABLE t2 (a INT) DYNAMIC_PARTITION_POLICY = (ENABLE = false, TIME_UNIT = 'day') PARTITION BY RANGE (a) (PARTITION p0 VALUES LESS THAN (10))", true, "CREATE TABLE `t2` (`a` INT) DYNAMIC_PARTITION_POLICY (ENABLE = false, TIME_UNIT = 'day') PARTITION BY RANGE (`a`) (PARTITION `p0` VALUES LESS THAN (10))"},
		{"create table t (c int) PARTITION BY HASH (c) PARTITIONS 32;", true, "CREATE TABLE `t` (`c` INT) PARTITION BY HASH (`c`) PARTITIONS 32"},
		// OceanBase WITH COLUMN GROUP (hybrid row/column store)
		{"create table t (c1 int primary key, c2 int) with column group (each column);", true, "CREATE TABLE `t` (`c1` INT PRIMARY KEY,`c2` INT) WITH COLUMN GROUP(EACH COLUMN)"},
		{"create table t (c1 int primary key, c2 int) with column group (all columns, each column);", true, "CREATE TABLE `t` (`c1` INT PRIMARY KEY,`c2` INT) WITH COLUMN GROUP(ALL COLUMNS, EACH COLUMN)"},
		{"create table t (pk int primary key) partition by hash(pk) partitions 128 with column group(all columns, each column);", true, "CREATE TABLE `t` (`pk` INT PRIMARY KEY) PARTITION BY HASH (`pk`) PARTITIONS 128 WITH COLUMN GROUP(ALL COLUMNS, EACH COLUMN)"},
...
ENGINE=INNODB PARTITION BY LINEAR HASH (a) PARTITIONS 1;`, true, "CREATE TABLE `t1` (`a` INT,`b` TIMESTAMP DEFAULT '0000-00-00 00:00:00') ENGINE = INNODB PARTITION BY LINEAR HASH (`a`) PARTITIONS 1"},
		...
		// OceanBase partition syntax support - SUBPARTITION TEMPLATE with RANGE (PARTITIONS before SUBPARTITION)
		{
			"CREATE TABLE t_m_rcrc(col1 INT,col2 INT) PARTITION BY RANGE COLUMNS(col1) SUBPARTITION BY RANGE COLUMNS(col2) SUBPARTITION TEMPLATE (SUBPARTITION mp0 VALUES LESS THAN(1000), SUBPARTITION mp1 VALUES LESS THAN(2000), SUBPARTITION mp2 VALUES LESS THAN(3000)) (PARTITION p0 VALUES LESS THAN(100), PARTITION p1 VALUES LESS THAN(200), PARTITION p2 VALUES LESS THAN(300))", true,
		...
		// OceanBase partition syntax support - SUBPARTITION TEMPLATE with LIST
		{
			"CREATE TABLE t_m_rclc(col1 INT,col2 INT) PARTITION BY RANGE COLUMNS(col1) SUBPARTITION BY LIST COLUMNS(col2) SUBPARTITION TEMPLATE (SUBPARTITION mp0 VALUES IN(1,3), SUBPARTITION mp1 VALUES IN(4,6), SUBPARTITION mp2 VALUES IN(7)) (PARTITION p0 VALUES LESS THAN(100), PARTITION p1 VALUES LESS THAN(200), PARTITION p2 VALUES LESS THAN(300))", true,
		...
		// OceanBase partition syntax support - SUBPARTITION TEMPLATE with HASH (PARTITIONS before SUBPARTITION)
		{
			"CREATE TABLE tbl2_m_hh(col1 INT,col2 INT,col3 INT) PARTITION BY HASH(col1) PARTITIONS 5 SUBPARTITION BY HASH(col2) SUBPARTITION TEMPLATE (SUBPARTITION sp0, SUBPARTITION sp1, SUBPARTITION sp2)", true,
			"CREATE TABLE `tbl2_m_hh` (`col1` INT,`col2` INT,`col3` INT) PARTITION BY HASH (`col1`) PARTITIONS 5 SUBPARTITION BY HASH (`col2`) SUBPARTITION TEMPLATE (SUBPARTITION `sp0`,SUBPARTITION `sp1`,SUBPARTITION `sp2`)",
		},

		// OceanBase partition syntax support - SUBPARTITION TEMPLATE with KEY (PARTITIONS before SUBPARTITION)
		{
			"CREATE TABLE tbl2_m_kk(col1 INT,col2 INT,col3 INT) PARTITION BY KEY(col1) PARTITIONS 5 SUBPARTITION BY KEY(col2) SUBPARTITION TEMPLATE (SUBPARTITION sp0, SUBPARTITION sp1, SUBPARTITION sp2)", true,
			"CREATE TABLE `tbl2_m_kk` (`col1` INT,`col2` INT,`col3` INT) PARTITION BY KEY (`col1`) PARTITIONS 5 SUBPARTITION BY KEY (`col2`) SUBPARTITION TEMPLATE (SUBPARTITION `sp0`,SUBPARTITION `sp1`,SUBPARTITION `sp2`)",
		},
		...
		{"CREATE TABLE t2_m_lcrc(col1 INT,col2 INT) PARTITION BY LIST COLUMNS(col1) SUBPARTITION BY RANGE COLUMNS(col2) SUBPARTITION TEMPLATE (SUBPARTITION mp0 VALUES LESS THAN(100), SUBPARTITION mp1 VALUES LESS THAN(200), SUBPARTITION mp2 VALUES LESS THAN(300)) (PARTITION p0 VALUES IN(1,3), PARTITION p1 VALUES IN(4,6), PARTITION p2 VALUES IN(7,9))", true, ""},
		{"CREATE TABLE t2_m_lclc(col1 INT,col2 INT) PARTITION BY LIST COLUMNS(col1) SUBPARTITION BY LIST COLUMNS(col2) SUBPARTITION TEMPLATE (SUBPARTITION mp0 VALUES IN(2), SUBPARTITION mp1 VALUES IN(5), SUBPARTITION mp2 VALUES IN(8)) (PARTITION p0 VALUES IN(1,3), PARTITION p1 VALUES IN(4,6), PARTITION p2 VALUES IN(7,9))", true, ""},
		...
		{"CREATE TABLE tbl2_m_hr(col1 INT,col2 INT,col3 INT) PARTITION BY HASH(col1) SUBPARTITION BY RANGE(col2) SUBPARTITION TEMPLATE (SUBPARTITION p0 VALUES LESS THAN(100), SUBPARTITION p1 VALUES LESS THAN(200), SUBPARTITION p2 VALUES LESS THAN(300)) PARTITIONS 5", true, ""},
		{"CREATE TABLE tbl2_m_hl(col1 INT,col2 INT,col3 INT) PARTITION BY HASH(col1) SUBPARTITION BY LIST(col2) SUBPARTITION TEMPLATE (SUBPARTITION p0 VALUES IN(1,3), SUBPARTITION p1 VALUES IN(4,6), SUBPARTITION p2 VALUES IN(7,9)) PARTITIONS 5", true, ""},
		{"CREATE TABLE tbl2_m_hk(col1 INT,col2 INT,col3 INT) PARTITION BY HASH(col1) SUBPARTITION BY KEY(col2) SUBPARTITION TEMPLATE (SUBPARTITION sp0, SUBPARTITION sp1, SUBPARTITION sp2) PARTITIONS 5", true, ""},
		{"CREATE TABLE tbl2_m_krc(col1 INT,col2 INT,col3 INT) PARTITION BY KEY(col1) SUBPARTITION BY RANGE COLUMNS(col2) SUBPARTITION TEMPLATE (SUBPARTITION sp0 VALUES LESS THAN(100), SUBPARTITION sp1 VALUES LESS THAN(200), SUBPARTITION sp2 VALUES LESS THAN(300)) (PARTITION p0, PARTITION p1, PARTITION p2)", true, ""},
		{"CREATE TABLE tbl2_m_klc(col1 INT,col2 INT,col3 INT) PARTITION BY KEY(col1) SUBPARTITION BY LIST COLUMNS(col2) SUBPARTITION TEMPLATE (SUBPARTITION p0 VALUES IN(1,3), SUBPARTITION p1 VALUES IN(4,6), SUBPARTITION p2 VALUES IN(7,9)) PARTITIONS 5", true, ""},
		{"CREATE TABLE tbl2_m_kh(col1 INT,col2 INT,col3 INT) PARTITION BY KEY(col1) SUBPARTITION BY HASH(col2) SUBPARTITION TEMPLATE (SUBPARTITION sp0, SUBPARTITION sp1, SUBPARTITION sp2) PARTITIONS 5", true, ""},
		...
		{"CREATE TABLE asfasfa (id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT, vin VARCHAR(20) NOT NULL DEFAULT '', object_key VARCHAR(256) NOT NULL DEFAULT '', car_type CHAR(32) NOT NULL DEFAULT '', unique_id BIGINT NOT NULL DEFAULT '0', data_type CHAR(12) NOT NULL DEFAULT '', status INT NOT NULL DEFAULT '0', PRIMARY KEY (id, car_type, unique_id), UNIQUE KEY uniq_key (unique_id, vin, data_type, car_type)) PARTITION BY RANGE(unique_id) SUBPARTITION BY KEY(car_type) SUBPARTITION TEMPLATE (SUBPARTITION p0,SUBPARTITION p1,SUBPARTITION p2,SUBPARTITION p3,SUBPARTITION p4,SUBPARTITION p5,SUBPARTITION p6,SUBPARTITION p7,SUBPARTITION p8,SUBPARTITION p9,SUBPARTITION p10,SUBPARTITION p11,SUBPARTITION p12,SUBPARTITION p13,SUBPARTITION p14,SUBPARTITION p15,SUBPARTITION p16,SUBPARTITION p17,SUBPARTITION p18,SUBPARTITION p19,SUBPARTITION p20,SUBPARTITION p21,SUBPARTITION p22,SUBPARTITION p23,SUBPARTITION p24,SUBPARTITION p25,SUBPARTITION p26,SUBPARTITION p27,SUBPARTITION p28,SUBPARTITION p29) (PARTITION M202301 VALUES LESS THAN (1675180800000), PARTITION M202302 VALUES LESS THAN (1677600000000), PARTITION M202303 VALUES LESS THAN (1680278400000), PARTITION M202304 VALUES LESS THAN (1682870400000), PARTITION M202305 VALUES LESS THAN (1685548800000), PARTITION M202306 VALUES LESS THAN (1688140800000))", true, ""},

		// OceanBase partition syntax support - PARTITIONS after SUBPARTITION TEMPLATE (previously unsupported)
		{
			"CREATE TABLE t1 (c1 INT,c2 INT) PARTITION BY HASH(c1) SUBPARTITION BY HASH(c2) SUBPARTITION TEMPLATE (SUBPARTITION p0, SUBPARTITION p1) PARTITIONS 5", true,
			"CREATE TABLE `t1` (`c1` INT,`c2` INT) PARTITION BY HASH (`c1`) PARTITIONS 5 SUBPARTITION BY HASH (`c2`) SUBPARTITION TEMPLATE (SUBPARTITION `p0`,SUBPARTITION `p1`)",
		},
		{
			"CREATE TABLE t2 (c1 INT,c2 INT) PARTITION BY RANGE(c1) SUBPARTITION BY RANGE(c2) SUBPARTITION TEMPLATE (SUBPARTITION sp0 VALUES LESS THAN (100), SUBPARTITION sp1 VALUES LESS THAN (200)) PARTITIONS 3 (PARTITION p0 VALUES LESS THAN (10), PARTITION p1 VALUES LESS THAN (20), PARTITION p2 VALUES LESS THAN (30))", true,
			"CREATE TABLE `t2` (`c1` INT,`c2` INT) PARTITION BY RANGE (`c1`) PARTITIONS 3 SUBPARTITION BY RANGE (`c2`) SUBPARTITION TEMPLATE (SUBPARTITION `sp0` VALUES LESS THAN (100),SUBPARTITION `sp1` VALUES LESS THAN (200)) (PARTITION `p0` VALUES LESS THAN (10),PARTITION `p1` VALUES LESS THAN (20),PARTITION `p2` VALUES LESS THAN (30))",
		},
		// OceanBase partition syntax support - Duplicate PARTITIONS should fail
		{
			"CREATE TABLE t1 (c1 INT,c2 INT) PARTITION BY HASH(c1) PARTITIONS 5 SUBPARTITION BY HASH(c2) SUBPARTITION TEMPLATE (SUBPARTITION p0, SUBPARTITION p1) PARTITIONS 5", false,
			"",
		},
	}
	s.RunTest(c, table)
